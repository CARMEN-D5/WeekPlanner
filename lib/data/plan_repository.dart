...
class PlanRepository {
  ...
  Future<TimelineModel> fetchTimeline() async {
    ...
    var blockRows =
        await database.query('timeline_blocks', orderBy: 'order_index ASC');
    ...
    await database.transaction((transaction) async {
      ...
      for (final entry in timeline.gapLabels.entries) {
        await transaction.insert(
            'timeline_gaps', {'after_order': entry.key, 'label': entry.value});
      }
    });
  }
  ...
  Future<void> splitSegment(String segmentId, int atMinute) async {
    ...
    final segment =
        timeline.segments.firstWhere((item) => item.id == segmentId);
    if (atMinute <= segment.startMinute || atMinute >= segment.endMinute)
      return;
    final database = await _db;
    await database.transaction((transaction) async {
      await transaction
          .delete('timeline_segments', where: 'id = ?', whereArgs: [segmentId]);
      await transaction.insert('timeline_segments', {
        'id': _uuid.v4(),
        'block_id': segment.blockId,
        'start_minute': segment.startMinute,
  ...
  Future<void> mergeSegmentWithNext(String segmentId) async {
    ...
    if (index < 0 ||
        ...
        segments[index + 1].startMinute != segment.endMinute) return;
    final next = segments[index + 1];
    final database = await _db;
    await database.transaction((transaction) async {
      await transaction.delete('timeline_segments',
          where: 'id IN (?, ?)', whereArgs: [segment.id, next.id]);
      await transaction.insert('timeline_segments', {
        'id': _uuid.v4(),
        'block_id': segment.blockId,
        'start_minute': segment.startMinute,
  ...
  Future<List<WeekTemplate>> fetchTemplates(String planId) async {
    ...
    final rows = await database
        .query('week_templates', where: 'plan_id = ?', whereArgs: [planId]);
  ...
  Future<void> savePlan(Plan plan, List<WeekTemplate> templates) async {
    ...
    final conflicts = (await fetchPlans())
        .where((candidate) =>
            ...
            _overlaps(plan.startDate, plan.endDate, candidate.startDate,
                candidate.endDate))
        .toList();
    ...
    await database.transaction((transaction) async {
      await transaction.insert('plans', _planRow(plan),
          conflictAlgorithm: ConflictAlgorithm.replace);
      await transaction
          .delete('week_templates', where: 'plan_id = ?', whereArgs: [plan.id]);
  ...
  Future<void> replaceConflictingAndSave(
      Plan plan, List<WeekTemplate> templates) async {
    _ensureTemplateTimesDoNotOverlap(templates);
    final conflicts = (await fetchPlans())
        .where((candidate) =>
            candidate.id != plan.id &&
    ...
    await database.transaction((transaction) async {
      for (final conflict in conflicts) {
        await transaction
            .delete('plans', where: 'id = ?', whereArgs: [conflict.id]);
        await transaction.delete('week_templates',
            where: 'plan_id = ?', whereArgs: [conflict.id]);
        await transaction.delete('task_instances',
            where: 'plan_id = ?', whereArgs: [conflict.id]);
      }
      await transaction.insert('plans', _planRow(plan),
          conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<void> deletePlan(String planId) async {
    ...
    await database.transaction((transaction) async {
      ...
      await transaction
          .delete('week_templates', where: 'plan_id = ?', whereArgs: [planId]);
      await transaction
          .delete('task_instances', where: 'plan_id = ?', whereArgs: [planId]);
    });
  }

  Future<void> saveInstance(PlanItem item) async {
    final database = await _db;
    await database.insert('task_instances', _itemRow(item),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> deleteInstance(String instanceId) async {
    final database = await _db;
    await database
        .delete('task_instances', where: 'id = ?', whereArgs: [instanceId]);
  }
  ...
  Map<String, Object?> _planRow(Plan plan) => {
        ...
        'finished_at':
            _storedLifecycleStatus(plan) == PlanLifecycleStatus.finished
  ...
  Plan _planFromRow(Map<String, Object?> row) => Plan(
        ...
        statisticsFinalizedAt:
            _parseNullableDate(row['statistics_finalized_at'] as String?),
      );
